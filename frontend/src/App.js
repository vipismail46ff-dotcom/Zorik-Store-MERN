...
function App() {
  ...
  const handleAddStockSubmit = async (e) => {
    ...
    try {
      ...
      const uploadRes = await axios.post('https://zorik-backend-api.onrender.com/api/upload', formData, {
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', backgroundColor: '#f9f9f9', minHeight: '100vh', paddingBottom: '50px', position: 'relative' }}>
      ...
      <div style={{ width: '85%', margin: '0 auto', textAlign: 'center' }}>
        ...
        {loading && <h3 style={{ color: 'blue', marginTop: '40px' }}>⏳ Loading products from Live Cloud...</h3>}
            ...
            <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '12px', width: '680px', maxHeight: '85vh', display: 'flex', flexDirection: 'column', textAlign: 'left', boxShadow: '0 10px 40px rgba(0,0,0,0.5)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #222', paddingBottom: '12px', marginBottom: '15px' }}>
                <div>
                  ...
                  <p style={{ margin: 0, color: '#00cc99', fontSize: '0.85rem', fontWeight: 'bold' }}>🟢 LIVE CLOUD PIPELINE ACTIVE</p>
                </div>
              ...
              <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                <button onClick={() => { setAdminTab('orders'); fetchAdminOrders(); }} style={{ flex: 1, padding: '12px', fontWeight: 'bold', backgroundColor: adminTab === 'orders' ? '#111' : '#f1f1f1', color: adminTab === 'orders' ? '#00ffcc' : '#444', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>📥 CUSTOMER ORDERS ({adminOrders.length})</button>
                <button onClick={() => setAdminTab('add_stock')} style={{ flex: 1, padding: '12px', fontWeight: 'bold', backgroundColor: adminTab === 'add_stock' ? '#111' : '#f1f1f1', color: adminTab === 'add_stock' ? '#00ffcc' : '#444', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>➕ UPLOAD NEW STOCK 🖼️</button>
              </div>
              ...
              <div style={{ overflowY: 'auto', flexGrow: 1, paddingRight: '5px' }}>
                {adminTab === 'orders' ? (
                    ...
                    adminOrders.length === 0 ? <p style={{ textAlign: 'center', color: '#777' }}>No orders found.</p> :
                      adminOrders.map((ord) => (
                        <div key={ord._id} style={{ border: '1px solid #ddd', borderRadius: '8px', padding: '15px', marginBottom: '15px', backgroundColor: '#fafafa' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px dashed #ccc', paddingBottom: '6px', marginBottom: '6px' }}><span style={{ fontWeight: 'bold', color: '#111' }}>Order #{ord.orderId}</span><span style={{ color: 'green', fontWeight: 'bold' }}>{ord.status || 'Order Placed'}</span></div>
                  ...
                  <form onSubmit={handleAddStockSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px', paddingTop: '5px' }}>
                    <div style={{ border: '2px dashed #00cc99', padding: '25px', textAlign: 'center', borderRadius: '10px', backgroundColor: '#f0fdf4', cursor: 'pointer' }}>
                      <label style={{ cursor: 'pointer', display: 'block' }}>
                        <div style={{ fontSize: '2.5rem', marginBottom: '5px' }}>📸</div>
                        <span style={{ fontWeight: 'bold', color: '#155724', fontSize: '1.05rem', display: 'block' }}>{selectedFile ? `✅ Ready: "${selectedFile.name}"` : "Choose from Gallery / Phone Camera"}</span>
                    ...
                    <div style={{ display: 'flex', gap: '15px' }}>
                      ...
                      <input type="number" required placeholder="Price ₹" value={newStock.price} onChange={(e) => setNewStock({ ...newStock, price: e.target.value })} style={{ flex: 1, padding: '10px', borderRadius: '6px', border: '1px solid #ccc', fontWeight: 'bold' }} />
                    </div>
                    ...
                    <button type="submit" disabled={isUploading} style={{ padding: '15px', backgroundColor: isUploading ? '#ccc' : '#111', color: isUploading ? '#666' : '#00ffcc', fontWeight: 'bold', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>{isUploading ? "☁️ UPLOADING..." : "⚡ PUBLISH TO LIVE STORE"}</button>
                  </form>
                )}
              </div>
            </div>
      ...
      {celebration.show && <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100vh', backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 10000 }}><div style={{ backgroundColor: 'white', padding: '40px', borderRadius: '20px', textAlign: 'center' }}><h1>🎉 WOOHOO!</h1><p>Order Placed Successfully</p><button onClick={() => { setCelebration({ show: false }); handleOpenAdminClick(); }} style={{ padding: '10px 20px', backgroundColor: '#111', color: '#00ffcc', border: 'none', borderRadius: '20px', fontWeight: 'bold', cursor: 'pointer' }}>VIEW IN ADMIN</button></div></div>}
    </div>
  );
}
